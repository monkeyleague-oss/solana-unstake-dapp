...
export const pinkPropsStyle: React.CSSProperties = {
    ...
    height: "4.5rem",
    boxShadow:
        "0 2px 6px 0 #110f2c, inset 0 1px 0 0 #e83c8d, inset 0 -1px 0 0 rgba(0, 0, 0, 0.5)",
};
...
export const StakingWidget: FC = () => {
    ...
    if (!walletState.publicKey) {
        Content =
            <>
                <h1 className='font-promt text-3xl md:text-6xl text-cream uppercase mr-auto md:mx-auto tracking-widest'>Staking</h1>
                <span className="text-[#c5c4ff] text-sm text-left md:text-center">Connect your wallet in order to see your staking data and to unstake
                </span>
            </>
    } else if (stakingOnChainData && 'error' in stakingOnChainData) {
        Content = (
            <div className="error-content font-semibold">
                <h1 className='default-content font-promt text-3xl md:text-6xl text-cream uppercase mr-auto md:mx-auto py-3'>Something went wrong...</h1>
        ...
        Content = (
            <>
                <h1 className='font-promt text-3xl md:text-6xl text-cream uppercase mr-auto md:mx-auto tracking-widest'>Staking Program Details</h1>
                <ul className=" py-10 staking-data-cards uppercase text-[#d2ceff] font-extrabold text-center">
                    {[
                        { t: 'User Locaed (MBS)', v: splitAmountWithCommas(stakingOnChainData?.userTotal?.toString()) },
                        { t: 'User Rewards (MBS)', v: splitAmountWithCommas(stakingOnChainData?.userRewards?.toString()) },
                        { t: 'Total program locked (MBS)', v: splitAmountWithCommas(stakingOnChainData?.vaultTotal.toString()) },
                ...
                {txnSignature ? (
                    <>
                        <h2 className='default-content font-promt text-3xl md:text-6xl text-cream uppercase mr-auto md:mx-auto'>Unstake succeeded! </h2>
                        <p>
                    ...
                    <button
                        ...
                        style={{ ...pinkPropsStyle, cursor: false === amount.gt(new BN(0)) ? "not-allowed" : "pointer" }}>
